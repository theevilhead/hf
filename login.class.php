...
class login{
	...
	function __construct($data){
		$this->db_connect('users');//pass the database name where user details table contains.
		$this->handle_data($data);
	}

	private function handle_data($stuff){
		// $this->user_id = 	mysqli_real_escape_string($stuff['uid']);
		$this->uemail = mysqli_real_escape_string($stuff['umail']);
		$this->upass =  mysqli_real_escape_string($stuff['upass']);
